Today we’ll be looking at slime molds. Which are awesome. Single-celled organisms with thousands / millions of nuclei.
...
Hopefully that code looks familiar to you! Now that the shader has been defined, we can add a second call to `sg.compute` as follows:
...
You'll notice we've already setup a second `DISPATCH_COUNT_2` variable
at the top of our JS file to control how many times this compute shader is called based on the width and height of our screen and grid size.
